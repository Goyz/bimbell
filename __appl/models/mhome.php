...
class mhome extends CI_Model{
	...
	function getdata($type="", $p1="", $p2="",$p3="",$p4=""){
		...
		switch($type){
			case "cek_voucher":
				$data=array();
				$data['msg']=0;//DEFAULT TIDAK ADA
				$data['res']='';
				$sql="SELECT A.*,B.nominal FROM tbl_voucher_detil A 
						LEFT JOIN tbl_voucher_header B ON A.tbl_voucher_header_id=B.id
						WHERE A.kode_voucher='".$p1."'";
						
				$ex=$this->db->query($sql)->row_array();
				if(isset($ex['kode_voucher'])){
					if($ex['flag']=='A'){
						$data['msg']=$this->isi_voucher($p1,$ex['nominal']);
					}else{
						//SUDAH DIPAKAI
						$data['msg']=2;
					}
			...
			case "cetak_voucher":
				$data=array();
				$id=$this->input->post('id');
				$sql="SELECT * FROM tbl_voucher_header WHERE id=".$id;
				$data['header']=$this->db->query($sql)->row_array();
				$sql="SELECT A.*,B.nominal FROM tbl_voucher_detil A 
						LEFT JOIN tbl_voucher_header B ON A.tbl_voucher_header_id=B.id
						WHERE tbl_voucher_header_id=".$id;
				$data['detil']=$this->db->query($sql)->result_array();
				return $data;
			break;
			case "get_poin":
				$data=array();
				$sql="SELECT * FROM tbl_poin WHERE cl_peserta_id=".$this->auth['peserta_id'];
				$data['poin']=$this->db->query($sql)->row_array();
				$sql="SELECT A.*, 
						CASE 
							WHEN A.flag_video='L' THEN C.cover
							WHEN A.flag_video='U' THEN C.cover
							WHEN A.flag_video='S' THEN C.cover
							WHEN A.flag_video='M' THEN B.cover
						END AS file_na,
						CASE 
							WHEN A.flag_video='L' THEN CONCAT('Video Latihan ',G.judul_sub_bab)
							WHEN A.flag_video='U' THEN CONCAT('Video UN ',G.judul_sub_bab)
							WHEN A.flag_video='S' THEN CONCAT('Video SBMPTN ',G.judul_sub_bab)
							WHEN A.flag_video='M' THEN CONCAT('Video Materi ',F.judul_sub_bab)
							ELSE 'Registrasi/TopUp'
						END AS ket_his,
						CASE 
							WHEN A.flag='R' THEN 'Registrasi'
							WHEN A.flag='T' THEN 'TopUp'
							ELSE 'Menonton Video'
						END as keterangan
							
					FROM tbl_histori_poin A
					LEFT JOIN tbl_file_materi B ON A.tbl_file_id=B.id
					LEFT JOIN tbl_file_bank_soal C ON A.tbl_file_id=C.id
					LEFT JOIN tbl_konten_materi D ON B.tbl_konten_materi_id=D.id
					LEFT JOIN tbl_bank_soal E ON C.tbl_bank_soal_id=E.id
					LEFT JOIN tbl_sub_bab_pelajaran F ON D.tbl_sub_bab_id=F.id
					LEFT JOIN tbl_sub_bab_pelajaran G ON E.tbl_sub_bab_id=G.id
					WHERE A.cl_peserta_id=".$this->auth['peserta_id'];
				$data['his']=$this->db->query($sql)->result_array();
				return $data;
			break;
			case "get_email":
				$data=array();
				$sql="SELECT * FROM cl_peserta WHERE Email IS NOT NULL";
				$data['siswa']=$this->db->query($sql)->result_array();
				$sql="SELECT * FROM cl_guru WHERE Email IS NOT NULL ";
				$data['guru']=$this->db->query($sql)->result_array();
				return $data;
			break;
			case "config":
			...
			case "cek_poin":
				...
				if(isset($res['poin'])){$poin=$res['poin'];}
				return $poin;
			break;
			case "get_video":
			...
			case "data_login":
				...
				if(isset($ex['nama_user'])){
					if($ex['cl_user_group_id']!=3){
						...
						$sql = "
							SELECT A.*,B.group_user,C.cl_kelas_id,D.kelas,C.id as peserta_id,C.Foto as foto_profil,C.Nama
							FROM tbl_user A 
							LEFT JOIN cl_user_group B ON A.cl_user_group_id=B.id
			...
			case "cek_user":
				if($p1=='get'){
					...
					$sql = " SELECT A.*	FROM tbl_user A LEFT JOIN cl_peserta B ON B.tbl_user_id=A.nama_user
							WHERE A.nama_user='".$this->db->escape_str($this->input->post('usr'))."' OR B.Email='".$this->input->post('email')."'";
							//echo $sql;
					$res=$this->db->query($sql)->row_array();
					if(!isset($res['nama_user'])){
						$data['msg']=1;
						$data['usr']=$this->db->escape_str($this->input->post('usr'));
			...
			break;
			case "tbl_voucher":
				$sql="SELECT * FROM tbl_voucher_header ";
			break;
			...
			case "tree_pelajaran":
				...
				case "l_soal":
					if($id_pel){
						...
						foreach($cat as $v){
							$child=array();
							if($v=='UN (Ujian Nasional)'){$flag='U';$flag_mod='un';}
							if($v=='SMBPTN'){$flag='S';$flag_mod='smbptn';}
							if($v=='Latihan Soal'){$flag='L';$flag_mod='banksoal';}
	...
	function simpansavedata($table,$data,$sts_crud){ //$sts_crud --> STATUS NYEE INSERT, UPDATE, DELETE
		...
		switch ($table){
			case "tbl_broadcast":
				//print_r($data);exit;
				$email=array();
				if(isset($data['email_siswa'])){
					foreach($data['email_siswa'] as $v){
						$email[]=$v;
					}
				}
				if(isset($data['email_guru'])){
					foreach($data['email_guru'] as $v){
						$email[]=$v;
					}
				}
			...
			case "tbl_config":
				...
				if(count($data['config'])>0){
					$data_config=array('update_date'=>date('Y-m-d H:i:s'),'update_by'=>$this->auth['nama_user']);
					foreach($data['config'] as $v){
						$data_config['deskripsi']=$v['deskripsi'];
						$data_config['val']=$v['val'];
						$this->db->update('tbl_config',$data_config,array('id'=>$v['id']));
					}
					if($this->db->trans_status() == false){
						$this->db->trans_rollback();
						return 0;
	...
	function simpan_reg($p1="",$p2=""){
		...
		if($p1=="act"){
			...
			$sql="SELECT A.id,B.nama_user 
				FROM cl_peserta A 
				LEFT JOIN tbl_user B ON A.tbl_user_id=B.nama_user
				WHERE A.tbl_user_id='".$p2."'";
				
			$ex_usr=$this->db->query($sql)->row_array();
			if(isset($ex_usr['id'])){
				$sql="SELECT * FROM tbl_poin WHERE cl_peserta_id=".$ex_usr['id'];
				$ex=$this->db->query($sql)->row_array();
				if(!isset($ex['cl_peserta_id'])){
					$data_poin=array('cl_peserta_id'=>$ex_usr['id'],
									 'poin'=>$this->seting['poin_reg'],
									 'create_date'=>date('Y-m-d H:i:s'),
									 'create_by'=>$p2,
									 
					);
					$data_his=array('cl_peserta_id'=>$ex_usr['id'],
									 'jml_poin'=>$this->seting['poin_reg'],
									 'poin_terpakai'=>0,
									 'sisa_poin'=>$this->seting['poin_reg'],
									 'create_date'=>date('Y-m-d H:i:s'),
									 'create_by'=>$p2,
									 'flag'=>'R'
									 
					);
					$this->db->insert('tbl_poin', $data_poin);
					$this->db->insert('tbl_histori_poin', $data_his);
				}
			}
		}else{
			$post = array();
	...
	function calc_poin(){
		...
		$flag=$this->input->post('flag');
		$flag_vid=$this->input->post('flag_vid');
		switch($flag_vid){
			case "kelas":$flag_vid='M';$potong=(int)$this->seting['poin_vid_materi'];break;
			case "un":$flag_vid='U';$potong=(int)$this->seting['poin_vid_un'];break;
			case "smbptn":$flag_vid='S';$potong=(int)$this->seting['poin_vid_smbptn'];break;
			case "soal":$flag_vid='L';$potong=(int)$this->seting['poin_vid_lat'];break;
			case "l_soal":$flag_vid='O';$potong=(int)$this->seting['poin_vid_lat'];break;
			case "banksoal":$flag_vid='L';$potong=(int)$this->seting['poin_vid_lat'];break;
		}
		$msg=array();
		$msg['msg']=0;
		$msg['poin']=0;
		$sql="SELECT * FROM tbl_poin WHERE cl_peserta_id=".$this->auth['peserta_id'];
		$poin=$this->db->query($sql)->row_array();
		if($flag=='get'){
			if(isset($poin['id'])){
				$msg['msg']=1;
				$msg['poin']=$poin['poin'];
			...
			if(isset($poin['id'])){
				//$pakai=1;
				$pakai=$potong;
				$sisa=(int)$poin['poin']-$pakai;
				$data_poin=array('poin'=>$sisa,
								 'update_date'=>date('Y-m-d H:i:s'),
								 'update_by'=>$this->auth['nama_user'],
									 
				);
				$data_his=array('cl_peserta_id'=>$this->auth['peserta_id'],
								 'jml_poin'=>$poin['poin'],
								 'poin_terpakai'=>$pakai,
								 'sisa_poin'=>$sisa,
								 'create_date'=>date('Y-m-d H:i:s'),
				...
				$this->db->update('tbl_poin', $data_poin,array('cl_peserta_id'=>$this->auth['peserta_id']));
				$this->db->insert('tbl_histori_poin', $data_his);
				$msg['msg']=1;
				$msg['poin']=$sisa;
			}
	...
	function generate_voucher($voucher){
		$this->db->trans_begin();
		$qty=$this->input->post('qty');
		$flag=$this->input->post('flag');
		$nominal=$this->input->post('nominal');
		$data_header=array('nominal'=>$nominal,'qty'=>$qty,'create_date'=>date('Y-m-d H:i:s'),'create_by'=>$this->auth['nama_user'],'flag'=>$flag);
		$this->db->insert('tbl_voucher_header',$data_header);
		$id=$this->db->insert_id();
		
		foreach($voucher as $v){
			$data_detil=array('tbl_voucher_header_id'=>$id,
							  'kode_voucher'=>$v,
							  'flag'=>'A'
			);
			$this->db->insert('tbl_voucher_detil',$data_detil);
		}
	...
	function isi_voucher($voucher,$nominal){
		$this->db->trans_begin();
		$post=array('kode_voucher'=>$voucher,
					'cl_peserta_id'=>$this->auth['peserta_id'],
					'create_date'=>date('Y-m-d H:i:s'),
					'create_by'=>$this->auth['nama_user']
				);
		
		if($this->db->insert('tbl_voucher_pakai',$post)){
			$sql="SELECT * FROM tbl_poin WHERE cl_peserta_id=".$this->auth['peserta_id'];
			$poin=$this->db->query($sql)->row_array();
			if(isset($poin['poin'])){
				$sisa=(int)$poin['poin']+(int)$nominal;
				$data_poin=array('poin'=>$sisa,
							 'update_date'=>date('Y-m-d H:i:s'),
							 'update_by'=>$this->auth['nama_user'],
								 
				);
				$data_voucher=array('flag'=>'T',//FLAG sudah DIPAKAI
							 'update_date'=>date('Y-m-d H:i:s'),
							 'update_by'=>$this->auth['nama_user'],
				...
				$this->db->update('tbl_poin', $data_poin,array('cl_peserta_id'=>$this->auth['peserta_id']));
				$this->db->update('tbl_voucher_detil', $data_voucher,array('kode_voucher'=>$voucher));
				$this->db->insert('tbl_histori_poin', $data_his);
			}
